...
func (suite *KeeperIntegrationTestSuite) TestWEDGENSWrapUnwrap_MultiDecimal() {
	tests := []struct {
		name    string
		chainID testconstants.ChainID
	}{
	...
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			...
			// Deploy WEDGENS contract
			wedgensAddr, err := suite.factory.DeployContract(
				sender.Priv,
				evmtypes.EvmTxArgs{},
				factory.ContractDeploymentData{
					Contract: contracts.WEDGENSContract,
				},
			)
			suite.Require().NoError(err)
			...
			_, err = suite.factory.ExecuteContractCall(
				...
				factory.CallArgs{
					ContractABI: contracts.WEDGENSContract.ABI,
					MethodName:  "deposit",
				},
			)
			suite.Require().NoError(err)
			...
			// Check WEDGENS balance == wrapAmount
			bal, err := utils.GetERC20Balance(suite.network, wedgensAddr, sender.Addr)
			suite.Require().NoError(err)
			suite.Require().Equal(amount.Cmp(bal), 0, "WEDGENS balance should match deposited amount (expected: %s, actual: %s)", amount, bal)
			...
			// Final WEDGENS balance should be 0
			bal, err = utils.GetERC20Balance(suite.network, wedgensAddr, sender.Addr)
			suite.Require().NoError(err)
